...
def open_default_webbrowser(url, protocol_prefix="http://"):
    """
    ...
    @param protocol_prefix: The internet protocol to use.
    @type protocol_prefix: str

    @return: True on success, False on failure.
    @rtype: bool
    """
    if not url.startswith(protocol_prefix):
        # If protocol is absent from the url, attach it, otherwise
        # the file `url` will be opened in Linux flavors.
        full_url = protocol_prefix + url
    else:
        full_url = url
